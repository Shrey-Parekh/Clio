  ...
  **The milestone.** Wake word through to natural speech, with a real capability wired end to end.
  ...
  - [x] **1.13** First real capability, timers - `clio/capabilities/timer.py` + `clio/orchestrator.py`. This is the task the handoff flagged as the real gap: every Phase 1 piece through 1.12 was built and verified independently, but nothing talked to anything else - `clio/__main__.py` still just started, logged, and slept. `Orchestrator` is where that stops being true.
    ...
    - **A new correctness concern, solved rather than just flagged:** a fired timer announces itself independently of whatever the orchestrator is doing at that moment. Since `BargeInSpeaker`/`ConversationSession` assume single-flight use of both the TTS engine and the shared mic stream, an announcement racing a live conversational reply would either garble audio output or corrupt the shared frame generator by consuming it from two places at once. Fixed with one `asyncio.Lock` (`_speak_lock`) that both the main conversation loop and `_announce()` acquire before speaking - an announcement due while Clio is mid-response simply waits its turn rather than colliding. Verified directly: two overlapping speak attempts never produced overlapping time spans.
