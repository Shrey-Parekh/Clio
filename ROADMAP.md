  ...
  **The milestone.** Wake word through to natural speech, with a real capability wired end to end.
  ...
  - [~] **1.5** Wake word - train custom openWakeWord models for the 6 configured phrases from synthetic speech, then run always-on with a tuned threshold and a clear acknowledgement. Measure the false-trigger rate over a normal day before calling it done.
    - Training environment built in WSL2/Ubuntu 24.04 (the pipeline is Linux-only - Piper TTS). Full pipeline verified end to end: generate clips -> augment -> train -> save `.onnx`. `hey clio` trained and validated on Windows.
    - **The earlier CPU warning in this task was wrong and is retracted.** It assumed each phrase costs a full independent model. Measured: 1 model = 1.95% of one core, 6 models = 2.28%. openWakeWord shares melspectrogram + embedding computation across models; each extra classifier head costs ~0.07%. Phrase count is effectively free at runtime - it costs training time (~8 min/phrase), not CPU.
    - **Training-reported "recall" is a misleading proxy.** The 20k-step `hey clio` model reported recall 0.446, but tested against real audio on Windows it scores 0.9365 on "hey clio" vs 0.0009-0.001 on "hey there" / "hey google" / "what is the time" - a ~1000x separation, comfortably clear of the 0.4 threshold. Judge these models by real inference, not the training metric.
