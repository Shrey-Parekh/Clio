  ...
  **The milestone.** Wake word through to natural speech, with a real capability wired end to end.
  ...
  - [x] **1.13** First real capability, timers - `clio/capabilities/timer.py` + `clio/orchestrator.py`. This is the task the handoff flagged as the real gap: every Phase 1 piece through 1.12 was built and verified independently, but nothing talked to anything else - `clio/__main__.py` still just started, logged, and slept. `Orchestrator` is where that stops being true.
    ...
    - **Known, not fixed - decide before Phase 2:** `ConversationMemory` is created per wake, so context resets every time the wake word is needed again ("what did you just tell me?" fails across wakes). Whether re-waking should continue the session or start fresh is a product call, not an obvious bug. Related: when you barge in, the *full* reply text is already in memory even though you only heard part of it, so Clio thinks she said more than you heard - properly fixing that needs the TTS layer to report how much actually played.
